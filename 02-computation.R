...
# summarise the split_corr and ssr_x using medians
...
#### Monte Carlo Alpha-Like Coefficient (MCALC) ####
# 

#1. helper function ----

generate_mcalc <- function(csv_contents, judging_session, seed = 123, splits = 100, ...) {
  print(str_glue("{judging_session} MCALC..."))
  
  # make sure the cache folder exists for this judging session
  dir.create(file.path("data-cache", judging_session), showWarnings = FALSE)
  splits_folder <- str_glue("data-cache/{judging_session}/mcalc/")
  dir.create(file.path(splits_folder), showWarnings = FALSE)
  ...
  for (i in splits_still_to_do) {
    ...
    # Group 1: sample the judges at random
    judge_group1 <- judgements_for_sirt %>%
      select(judge) %>%
      distinct() %>%
      slice_sample(prop = 0.5)
    ...
    # Run BTM
    judgements1 <- judgements_for_sirt %>% slice_sample(prop = 1, replace = TRUE) %>% select(-judge)
    judgements2 <- judgements_for_sirt %>% slice_sample(prop = 1, replace = TRUE) %>% select(-judge)
    ...
    # Save results
    write_csv(merged_effects, file = str_glue("data-cache/{judging_session}/mcalc/split{i}.csv"))
  }
}

#2. generate the simulated datasets ----

pmap_dfr(judgement_data_tidy %>% filter(!is.na(observed_N_A)), generate_mcalc, splits = 150)

#3. read the cached results ----

compute_mcalc_cors_from_cache <- function(judging_session, ...) {
  fs::dir_ls(str_glue("data-cache/{judging_session}/mcalc/")) %>% 
    vroom::vroom(id = "path", show_col_types = FALSE) %>% 
    nest(data = !path) %>% 
    mutate(mcalc_corr = map_dbl(data, ~ cor(.x$theta.x, .x$theta.y, method = "pearson"))) %>% 
    # compute the sepG and SSR values for one of the split halves, to produce ssr_x
    mutate(G_x = map_dbl(data, ~ sd(.x$theta.x) / sqrt(mean(.x$se.theta.x ^ 2)) ),
           ssr_x = G_x ^ 2 / (1 + G_x ^ 2)
    ) %>% 
    select(-data) %>% 
    write_csv(str_glue("data-cache/{judging_session}/mcalc.csv"))
}

# if you just want to do one ad-hoc:
compute_mcalc_cors_from_cache(judging_session = "Bisson2016_algebra")

# Loop through all the judging sessions and generate the mcalc.csv if it doesn't exist yet
judgement_data_tidy %>% 
  pwalk(function(...) {
    args <- list(...) # get the list of named arguments
    print(str_glue("{args$judging_session}"))
    if(is.na(args$observed_N_A)) {
      ...
      if(file.exists(str_glue("data-cache/{args$judging_session}/mcalc.csv"))) {
        print(str_glue("  ✔ already done"))
      } else {
        compute_mcalc_cors_from_cache(judging_session = args$judging_session)
        print(str_glue("  ✔✔ done"))
      }
    }
  })

mcalc_data <- tibble(path = fs::dir_ls("data-cache", recurse = TRUE, glob = "*mcalc.csv")) %>% 
  separate(path, into = c(NA, "judging_session", NA), sep = "/", remove = FALSE) %>% 
  mutate(mcalc_corr = map(path, vroom::vroom, delim = ",", show_col_types = FALSE)) %>% 
  select(-path) %>% 
  unnest(cols = mcalc_corr) %>% 
  separate(path, into = c(NA, "iteration"), sep = "mcalc/split") %>% 
  mutate(iteration = str_remove(iteration, ".csv"))

# keep a cached copy of the mcalc_corr and ssr_x for each split
mcalc_data %>% select(-G_x) %>% write_csv("data-cache/mcalc_summary_stats.csv")

# summarise mcalc using medians
mcalc_values <- mcalc_data %>% 
  group_by(judging_session) %>%
  summarise(median_mcalc_corr = median(mcalc_corr))
...
meta_analysis_data %>% 
  left_join(mcalc_values) %>% 
  ggplot(aes(x = median_ssr_x, y = median_mcalc_corr)) +
  geom_point() +
...
mcalc_data %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x = ssr_x, y = mcalc_corr)) +
  geom_point() +
