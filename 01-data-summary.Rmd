...
sessions <- yaml::read_yaml("data/00-judging_sessions.yml")

# tibble(
#   project_id = map_chr(sessions, "project_id"),
#   citation = map_chr(sessions, "citation"),
#   doi = map_chr(sessions, "doi"),
#   open_data = map_chr(sessions, "open_data"),
#   republish = map_lgl(sessions, "republish"),
#   judging_sessions = map(sessions, "judging_sessions")
# ) %>%
...
* `republish` records whether we have permission to publish the judgement data in our open dataset

```{r}
all_sessions %>% 
  select(project_id, citation, doi, open_data, republish) %>% 
...
```{r}
all_sessions %>% 
  select(-citation, -doi, -open_data, -republish) %>%
  kable("html", escape = FALSE) %>%
  kable_styling() %>%
  ...
  mutate(
    csv_contents = map(csv_contents, function(df) {
      if (all(has_name(df, c("judge", "candidate_chosen", "candidate_not_chosen")))) {
        ...
        warning("Could not find suitable CJ decision columns")
